            ...
            {item.paid_status === 1 ? (
                <>
                    <TouchableOpacity
                        ...
                        style={{ backgroundColor: '#88da09', padding: 10, borderRadius: 5, marginBottom: 10 }}
                    >
                    ...
                    <TouchableOpacity
                        onPress={() => {
                            console.log("Item Object:", item); // Log the item object
                            Alert.alert(
                                'Confirm Deletion',
                                `Are you sure you want to delete record with ID: ${item?.customer_id || 'Unknown'}?`,
                                [
                                    {
                                        text: 'Cancel',
                                        style: 'cancel',
                                    },
                                    {
                                        ...
                                        onPress: () => {
                                            ...
                                            const deleteRentalQuery = 'DELETE FROM Rentals WHERE customer_id = ?';
                                            ...
                                            db.transaction((tx) => {
                                                tx.executeSql(
                                                    ...
                                                    [item.customer_id],
                                                    (_, result) => {
                                                        console.log(`Deleted rental record for customer with ID: ${item.customer_id}`);
                                                    },
                                                    (error) => {
                                                        console.error('SQL error for rental deletion:', error);
                                                    }
                                                );
                                                ...
                                                tx.executeSql(
                                                    ...
                                                    [item.customer_id],
                                                    (_, result) => {
                                                        console.log(`Deleted customer record with ID: ${item.customer_id}`);
                                                        // Refresh the list after deletion
                                                        fetchCustomers();
                                                    },
                                                    (error) => {
                                                        console.error('SQL error for customer deletion:', error);
                                                    }
                                                );
                                            });
                                        },
                    ...
                    >
                        <Text style={{ color: 'white', fontWeight: '600', textAlign: 'center', fontSize: 16 }}>Delete Record</Text>
                    </TouchableOpacity>
                </>
            ) : (
                <>
